...
import { inspectContact, closeInspectContact, formatCoord } from '../inspectDossier.js';
import { vesselInspectModel } from './aisIdentity.js';
...
function applyAisFeedSnapshot(viewer, payload) {
  ...
  if (typeof payload?.source === 'string' && payload.source.trim()) {
    state.feedSource = payload.source.trim();
  }
...
function normalizeVessel(row) {
  ...
  return {
    ...
    navStatus: finiteNumber(row.navStatus ?? row.nav_status),
...
function publishVesselOverlayEntries(entries) {
  ...
  _vesselOverlayHost.setEntries(
    ...
    entries.map((entry) => {
      ...
      return {
        ...card,
        accessibilityLabel: `Inspect vessel ${card.title}, MMSI ${mmsi}`,
        activate: () => {
...
/** Select one live vessel and open the inspect dossier. Clicking does not move the camera. */
function selectAndFocusVessel(record) {
  ...
  inspectContact(buildVesselInspectPayload(record));
  return true;
}

function buildVesselInspectPayload(record) {
  const model = vesselInspectModel(record, {
    normalizeType: normalizeVesselType,
    positionText: formatCoord(record.lat, record.lon),
  });
  return {
    kind: 'vessel',
    id: record.mmsi,
    title: String(record.name || record.mmsi || 'VESSEL').trim().toUpperCase(),
    ...model,
  };
}
...
function registerSelectedContext(record) {
  ...
  try {
    return registerEntityContext(record, {
      ...
      source: state.feedSource || 'Live AIS',
