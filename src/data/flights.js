...
/** @type {string|null} ICAO currently shown in the inspect dossier (no camera follow). */
let _inspectedIcao = null;
...
function _requestTypeEnrichment(icao24, priority = false) {
  ...
  _enqueueEnrich(`t:${icao24}`, `/api/adsbdb/type/${icao24.toLowerCase()}`, (data) => {
    ...
    if (icao24 === _inspectedIcao) _inspectFlight(icao24, { quiet: true });
  }, priority);
}
...
function buildFlightInspectPayload(icao24) {
  const info = _flightData.get(icao24) || {};
  const heading = Number(info.true_track);
  return {
    kind: 'aircraft',
    id: icao24,
    hex: icao24,
    callsign: info.callsign || '',
    classification: 'CIVIL AIRCRAFT',
    title: String(info.callsign || info.registration || icao24).trim().toUpperCase(),
    origin: info.route?.origin ? formatAirport(info.route.origin) : (info.originCountry || 'UNKNOWN'),
    destination: info.route?.destination ? formatAirport(info.route.destination) : 'UNKNOWN',
    fields: [
      { label: 'ICAO24', value: String(icao24).toUpperCase() },
      { label: 'CALLSIGN', value: info.callsign || 'UNKNOWN' },
      { label: 'REGISTRATION', value: info.registration || 'UNKNOWN' },
      { label: 'AIRFRAME', value: info.typeName || info.klass || 'UNKNOWN' },
      { label: 'OPERATOR', value: info.airline || 'UNKNOWN' },
      { label: 'COUNTRY', value: info.originCountry || 'UNKNOWN' },
      { label: 'ALTITUDE', value: formatFeetFromMeters(info.altitude) },
      { label: 'SPEED', value: formatKnotsFromMps(info.velocity) },
      { label: 'HEADING', value: Number.isFinite(heading) ? `${Math.round(heading)}°` : 'UNKNOWN' },
      { label: 'POSITION', value: formatCoord(info.rawLat, info.rawLon) },
      { label: 'STATUS', value: info.onGround ? 'ON GROUND' : 'AIRBORNE' },
    ],
  };
}

function _inspectFlight(icao24, { quiet = false } = {}) {
  if (!_flightData.has(icao24) && !_billboards.has(icao24)) return;
  if (!quiet && _trackedIcao) _clearTracking(false, { origin: 'user' });
  _inspectedIcao = icao24;
  if (!quiet) {
    _requestTypeEnrichment(icao24, true);
    _requestRouteEnrichment(icao24);
  }
  inspectContact(buildFlightInspectPayload(icao24));
}
...
const flightsLayer = {
  ...
  getStats() {
    ...
    return {
      ...
      source: _lastSource,
      coverage: _lastCoverage,
      // Live adsb.lol ADS-B is a real aircraft feed, not simulated traffic.
      fallback: /\badsb\.lol\b/i.test(String(_lastSource || '')) ? false : undefined,
    };
  },
};
...
function _onKeyDown(e) {
  ...
  if (e.key === 'Escape' && _inspectedIcao) {
    _inspectedIcao = null;
    closeInspectContact();
  }
}
...
function _installClickHandler(viewer) {
  ...
  bindTrackingClickGesture(_clickHandler, (click, gesture) => {
    ...
    if (picked) {
      // Clicking the tracked entity itself — inspect, do not zoom again.
      if (picked.id === _trackedEntity && _trackedIcao) {
        _inspectFlight(_trackedIcao);
        return;
      }
