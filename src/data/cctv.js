...
const PROJECTION_ACTIVE_REFRESH_MS = 15000;
...
const CAMERA_SEEDS = [
  ...
  { id: 'london-westminster', cityId: 'london', poiIndex: 2, label: 'Westminster Bridge', offsetNorthM: -90, offsetEastM: 40, headingDeg: 10, fovDeg: 72, rangeM: 680, elevationM: 22 },
  { id: 'london-tower-bridge', cityId: 'london', poiIndex: 0, label: 'Tower Bridge Approach', offsetNorthM: 60, offsetEastM: -40, headingDeg: 250, fovDeg: 70, rangeM: 640, elevationM: 24 },
  { id: 'london-oxford-circus', cityId: 'london', poiIndex: 2, label: 'Oxford Circus', offsetNorthM: 520, offsetEastM: -280, headingDeg: 90, fovDeg: 68, rangeM: 520, elevationM: 20 },
...
async function loadCameraSources() {
  try {
    const resp = await fetch(SOURCE_ENDPOINT, {
      cache: 'no-store',
...
/**
 * Live packs win on id collision; seed cameras fill cities the live catalog
 * does not cover so the layer is never an empty globe.
 * @param {Object[]} live
 * @param {Object[]} seeds
 * @returns {Object[]}
 */
function mergeCameraCatalog(live, seeds) {
  const byId = new Map();
  for (const camera of Array.isArray(seeds) ? seeds : []) {
    if (camera?.id) byId.set(camera.id, camera);
  }
  for (const camera of Array.isArray(live) ? live : []) {
    if (camera?.id) byId.set(camera.id, camera);
  }
  return [...byId.values()];
}
...
function setPlaneVisible(runtime, visible) {
  if (!runtime) return;
  const show = !!visible && projectionMediaReady(runtime);
  if (runtime.planeEntity) runtime.planeEntity.show = show;
  if (show && runtime.overlayEntry && runtime.cameraId) {
...
function createProjectionRuntime(record) {
  ...
  if (mode === 'video') {
    ...
    video.addEventListener('canplay', () => {
      video.play().catch(() => {});
      if (_enabled && _showProjection && runtime.cameraId === _activeCameraId) {
 ...
 */
function projectionMediaReady(runtime) {
  if (!runtime) return false;
  if (runtime.mode === 'video') {
    return !!runtime.video && runtime.video.readyState >= 2 && runtime.video.videoWidth > 0;
  }
  return runtime.imageReady === true && !!runtime.image;
}

function refreshProjectionImage(record, force = false) {
  const runtime = record?.projection;
  if (!runtime || runtime.mode !== 'image') return;
  ...
  incoming.src = `${frameUrl}${sep}projTs=${Math.floor(now / refreshMs)}`;
}
...
function drawProjectionFrame(record) {
  ...
  if (runtime.mode === 'video') {
    // Cesium samples the <video> element directly. Do not blit to canvas or
    // retarget src here — both blank the monitor plane for a frame.
    return;
  }
...
export function refreshCoverageStyles() {
  ...
  for (const record of _records) {
    ...
    // wireframe only.
    const mediaReady = projectionMediaReady(record.projection);
    const planeShowing = !!(_enabled && _showProjection && isActive && mediaReady);
...
const cctvLayer = {
  ...
  async init(viewer) {
    ...
    const catalog = buildCatalogFromSources(sources).filter((camera) => camera.feedConfigured);
    ...
    for (let i = 0; i < catalog.length; i++) {
      ...
      const billboard = _billboards.add({
        ...
        scaleByDistance: new Cesium.NearFarScalar(350, 1.45, 2_500_000, 0.85),
      });
  ...
  enable() {
    ...
    if (_records.length) {
      _activeCameraId = nearestCameraIdToViewer() || _activeCameraId || _records[0].camera.id;
      _autoHopSuspended = false;
    }
