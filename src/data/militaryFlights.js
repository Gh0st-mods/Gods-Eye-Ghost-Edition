...
function buildMilitaryInspectPayload(icao24) {
  const info = _flightData.get(icao24) || {};
  const heading = Number(info.track);
  const altitudeM = Number.isFinite(info.altitudeFt) ? info.altitudeFt * 0.3048 : info.renderAltitudeM;
  return {
    kind: 'aircraft',
    id: icao24,
    hex: icao24,
    callsign: info.callsign || '',
    classification: 'MILITARY AIRCRAFT',
    ...
    fields: [
      ...
      { label: 'AIRFRAME', value: info.type || info.klass || 'UNKNOWN' },
      { label: 'OPERATOR', value: info.operator || 'UNKNOWN' },
      { label: 'ALTITUDE', value: Number.isFinite(info.altitudeFt)
        ? `${Math.round(info.altitudeFt).toLocaleString('en-US')} FT`
        : formatFeetFromMeters(altitudeM) },
      { label: 'SPEED', value: formatKnotsFromMps(info.speedMps) },
...
function _inspectFlight(icao24, { quiet = false } = {}) {
  ...
  inspectContact(buildMilitaryInspectPayload(icao24));
}
