...
import { INSPECT_CONTACT_EVENT } from '../inspectDossier.js';
...
test('vessel real layer lifecycle publishes protected selection and leaves no stale host cards', () => {
  ...
  globalThis.document = { getElementById: () => null };
  const inspects = [];
  globalThis.window.addEventListener('gev:inspect-contact', (event) => {
    inspects.push(event.detail);
  });
  ...
  try {
    ...
    assert.match(publication.entries[0].accessibilityLabel, /Inspect vessel EVER GIVEN, MMSI 353136000/);
    assert.equal(publication.entries[0].activate(), true);
    assert.equal(inspects.length, 1);
    assert.equal(inspects[0].id, '353136000');
...
test('vessel interaction wire: selecting a vessel by click opens inspect without moving the camera', () => {
  ...
  harness.windowTarget.addEventListener(
    ...
    (event) => inspects.push(event.detail),
  );
  try {
    harness.handler.click({ position: { x: 10, y: 20 } });
    assert.equal(aisLiveVesselsLayer.getSelectedInfo()?.mmsi, harness.record.mmsi);
    ...
    assert.equal(inspects.length, 1);
    assert.equal(inspects[0].kind, 'vessel');
    assert.equal(inspects[0].id, harness.record.mmsi);
  } finally {
    harness.cleanup();
  }
});
...
test('vessel interaction wire: clicking a card selects and inspects, like the sprite', () => {
...
test('vessel interaction wire: clicking the selected vessel card reopens inspect exactly once', () => {
  const harness = installWireHarness({ primitive: {}, content: {}, featureId: 0 });
  const inspects = [];
  harness.windowTarget.addEventListener(
    INSPECT_CONTACT_EVENT,
