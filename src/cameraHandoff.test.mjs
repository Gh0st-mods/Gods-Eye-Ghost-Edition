...
test('vessel and fire layers announce valid clicks and never fly cameras', () => {
  assert.match(vessels, /inspectContact\(/);
  assert.match(firms, /requestWorldFocus\(\{/);
  ...
  assert.match(vesselFocus, /inspectContact\(/);
  assert.doesNotMatch(vesselFocus, /requestWorldFocus\(\{/);
