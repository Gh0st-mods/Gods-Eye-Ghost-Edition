...
export class MapStackController {
  constructor(viewer, {
    ...
    this._activeId = googleTileset ? initialStack : 'esri-aerial';
    ...
    if (!this.getStack(this._activeId) || !this.isStackAvailable(this._activeId)) {
      this._activeId = googleTileset ? 'photoreal' : 'esri-aerial';
    }
  }
  ...
  async _activateGlobeStack(stack, gen) {
    ...
    // Imagery first, terrain second. Re:Earth can hang on a blocked network
    // and used to freeze the whole loading screen. Use a flat ellipsoid now
    // and upgrade in the background when a token or Re:Earth is available.
    if (this.cesiumToken) {
      await this._setWorldTerrainEnabled(true, gen);
    } else {
      this.viewer.terrainProvider = new Cesium.EllipsoidTerrainProvider();
      this._terrainMode = 'keyless';
      void this._upgradeKeylessTerrain(gen);
    }
  }
   ...
   */
  async _upgradeKeylessTerrain(gen) {
    const provider = await this._getKeylessTerrainProvider();
    if (gen != null && gen !== this._switchGen) return;
  ...
  async _getKeylessTerrainProvider() {
    ...
    try {
      const TERRAIN_TIMEOUT_MS = 4000;
      this._reearthTerrainProvider = await Promise.race([
        Cesium.CesiumTerrainProvider.fromUrl(REEARTH_TERRAIN_URL),
        new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Re:Earth terrain timed out')), TERRAIN_TIMEOUT_MS);
        }),
      ]);
    } catch (error) {
