...
export const CITY_POIS = {
  ...
  southend: {
    ...
    groundElevation: 20,
    viewBounds: { southwest: { lat: 51.52, lng: 0.64 }, northeast: { lat: 51.58, lng: 0.75 } },
    pois: [
      { name: 'Southend Pier', lat: 51.5338, lon: 0.7167, alt: 500, pitch: -25, heading: 180, buildingHeight: 20 },
      { name: 'Victoria Avenue', lat: 51.5452, lon: 0.7106, alt: 450, pitch: -28, heading: 0, buildingHeight: 25 },
      { name: 'Cuckoo Corner', lat: 51.5518, lon: 0.7055, alt: 500, pitch: -28, heading: 90, buildingHeight: 15 },
      { name: 'The Bell', lat: 51.5550, lon: 0.6955, alt: 450, pitch: -25, heading: 270, buildingHeight: 15 },
      { name: 'Kent Elms', lat: 51.5559, lon: 0.6655, alt: 500, pitch: -28, heading: 90, buildingHeight: 15 },
    ],
  },
  paris: {
    name: 'Paris',
...
async function geocodeNominatim(query) {
  ...
  return {
    ...
    types: [hit.type, hit.class, hit.addresstype].filter(Boolean),
    viewport: [south, north, west, east].every(Number.isFinite)
...
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  if (apiKey) {
    ...
    if (nominatim) {
      result = {
        formatted_address: nominatim.label,
        geometry: {
          location: { lat: nominatim.lat, lng: nominatim.lng },
          bounds: nominatim.viewport,
          viewport: nominatim.viewport,
        },
        types: nominatim.types,
      };
    }
  }
